 ...
 */
export function extractCommand(content: string, commandPrefix: string = "@toss"): string | null {
 ...
 */
export function isCommand(message: DecodedMessage, commandPrefix: string = "@toss"): boolean {
 ...
 */
export function isExplicitCommand(commandName: string, allowedCommands: string[] = ["help"]): boolean {
  return allowedCommands.map(cmd => cmd.toLowerCase()).includes(commandName.toLowerCase());
}
 ...
 */
export function shouldProcessMessage(message: DecodedMessage, options: AgentOptions): boolean {
